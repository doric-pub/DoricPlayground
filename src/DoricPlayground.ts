...
class DoricPlayground extends Panel {
    ...
    prefix() {
        const extra = this.getInitData()
        if (extra && (extra as any).href) {
            return (extra as any).href
        }
        return "https://raw.githubusercontent.com/doric-pub/DoricPlayground/master/bundle/"
    }
                    ...
                    ...files.map(e =>
                        vlayout(
                            ...
                            ],
                            {
                                ...
                                onClick: () => {
                                    navigator(context).push(this.prefix() + e.url, {
                                        alias: e.alias,
                                    })
                                },
                            }
